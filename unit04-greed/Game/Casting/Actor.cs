...
namespace unit04_greed.Game.Casting
{
    ...
    public class Actor
    {
        ...
        /// <summary>
        /// Moves the artifact to its next position according to its speed. Will wrap the position 
        /// from one side of the screen to the other when it reaches the maximum y value.
        /// </summary>
        /// <param name="maxX">The maximum x value.</param>
        /// <param name="maxY">The maximum y value.</param>
        public void MoveArtifact(int maxX, int maxY)
        {
            int x = _position.GetX();
            int y = ((_position.GetY() + _speed) + maxY) % maxY;
            _position = new Point(x, y);
        }
        ...
        /// <summary>
        /// Sets the actor's speed to the given value.
        /// </summary>
        /// <param name="speed">The given velocity.</param>
        public void SetSpeed(int speed)
        {
            if (speed <= 0)
            {
                Random random = new Random();
                speed = random.Next(1,7);
            }
            this._speed = speed;
        }
